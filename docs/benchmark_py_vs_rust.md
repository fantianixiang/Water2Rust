...
| fclass | 0.4 s | 7.1 s |
| edge | 0.0 s | 0.0 s |
| hydro | 53.8 s | 104.7 s |
| 合计 | 54.4 s | 111.8 s |

> ⚠️ **全量两侧条件不同，wall time 不可直接比较**（见下节）。
...
| 并行 | **4 个瓦片 worker**（multiprocessing） | 瓦片循环**单线程** |
| 输出分辨率 | 半分辨率 | 全分辨率 |

- Python 的 53.8 s 是在**约 1/4 像素量 + 4 核并行**下取得；折算到「全分辨率 · 单线程」量级远大于此。
- Rust 的 104.7 s 是**全分辨率 · 单线程**；若同样降采样并用 rayon 多核，预计可降到十几秒量级。
- 因此「Python 54s 比 Rust 112s 快」是**表象**，根因是默认配置（降采样 + 并行）不同，而非算法效率。裁剪数据（同条件）已显示 Rust hydro 求解更快。

其它观察：
- **fclass Rust 偏慢**（裁剪 2.0 s / 全量 7.1 s，Python 仅 0.1 / 0.4 s）：Rust fclass 每次**急加载整个参考库** `waters_china.gpkg`（rusqlite 读全图层 + rstar 建索引），Python 按需读取。这是 fclass 独立的优化点，与 hydro 修复无关。
...
- **公平对比结论**：同条件（裁剪数据）Rust hydro 更快；全量 wall time 因 Python 降采样 + 4 核并行而领先，属配置差异非算法差异。
- **Rust 后续可选优化**（使全量也全面领先）：
  1. hydro 瓦片循环 **rayon 并行**（对齐 Python 的多 worker）；
  2. 超预算时提供**可选降采样**开关（对齐 Python 默认，或保留全分辨率为高质量模式）；
  3. **fclass 惰性 / 空间过滤读取**参考库，消除急加载开销。
